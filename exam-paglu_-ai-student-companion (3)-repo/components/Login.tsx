...
const Login: React.FC<LoginProps> = ({ onLoginSuccess }) => {
  ...
  const handleTryWithoutLogin = () => {
    setLoading(true);
    setTimeout(() => {
      onLoginSuccess(); // Directly allow access
      setLoading(false);
      ...
      <div className="bg-white p-10 rounded-3xl shadow-2xl w-full max-w-md border border-slate-100">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-orange-600 text-white py-4 rounded-2xl font-bold hover:bg-orange-700 transition-all shadow-xl shadow-orange-100 flex items-center justify-center text-lg active:scale-[0.98] disabled:bg-slate-200"
          >
            {loading ? (
        ...
        <div className="mt-6 text-center">
          <button
            ...
            disabled={loading}
            className="bg-slate-100 text-slate-600 px-6 py-3 rounded-xl font-bold hover:bg-slate-200 transition-all border border-slate-200 text-sm active:scale-[0.98] w-full disabled:bg-slate-200"
          >
            Try without Login
          </button>
        </div>
